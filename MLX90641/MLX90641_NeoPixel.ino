// AMLX90641_NeoPixel.ino
// Prints 16x12 temperature matrix to 16x16 NeoPixel display
// Author: D. Dubins
// Date: 08-Dec-25
//
...
#define LED_PIN 19    // GPIO19 (D19) for LED pin to NeoPixel
#define NUM_LEDS 192
...
unsigned long timer = 0;  // to time samples
...
#define CAL_SLOPE 2.64896693658985          // Slope of T_meas vs. T_o calibration curve (post-hoc calibration)
#define EEPROM_WORDS 832                    // MLX90641 EEPROM size in 16-bit words
uint16_t eeData[EEPROM_WORDS];              // to hold the EEPROM contents

float Vdd = 0.0;                     // to hold calculated Vdd (measured sensor operating voltage)
int16_t Vdd_25 = 0;                  // to store Vdd at 25°C
int16_t K_Vdd = 0;                   // to store K_Vdd
float Ta = 0.0;                      // calculated Ta (ambient temperature)
float Kgain = 0.0;                   // Kgain coefficient
...
int16_t CT1, CT2, CT3, CT4, CT5;     // Corner temperatures
uint16_t CT6, CT7, CT8;              // Corner temperatures
float KsTo1, KsTo2, KsTo3, KsTo4;    // KsTo coefficients
float KsTo5, KsTo6, KsTo7, KsTo8;
float Alpha_cr1, Alpha_cr2;  // Alpha correction coefficients for each range
float Alpha_cr3, Alpha_cr4;
float Alpha_cr5, Alpha_cr6;
float Alpha_cr7, Alpha_cr8;
float alpha_reference_row1;  // Alpha references for sensitivity adjustment
float alpha_reference_row2;
float alpha_reference_row3;
float alpha_reference_row4;
float alpha_reference_row5;
float alpha_reference_row6;
...
float V_IR_compensated[NUM_PIXELS] = { 0 };  // V_IR_compensated values
float T_o[NUM_PIXELS] = { 0.0 };             // Matrix to hold final T_o[i] values

// Read the device EEPROM
bool readEEPROMBlock(uint16_t startAddr, uint16_t numWords, uint16_t *dest) {
...
uint16_t readEEPROM_unsigned(uint16_t addr) {
...
int16_t readEEPROM_signed(uint16_t addr) {  // helper function to read signed integer from eeData
  return (int16_t)readEEPROM_unsigned(addr);
}

float readVdd() {  //(From 11.1.1, worked example in 11.2.2.2)
...
float readTa() {  // Read ambient temperature, datasheet, 11.1.2
  ...
  int16_t Vdd = readAddr_signed(0x05AA);                   // Read Vdd again from RAM, address 0x05AA
  if (Vdd > 32767) Vdd = Vdd - 65536;                      // impose limits
  float dV = ((float)Vdd - (float)Vdd_25) / (float)K_Vdd;  // calculate the change in voltage dV
...
void readKta() {
  ...
  for (uint16_t i = 0; i < 192; i++) {
    ...
    Kta[i] = ((float)Kta_EE * two_to_the(Kta_scale2) + (float)Kta_average) / two_to_the(Kta_scale1);
  }
#ifdef DEBUG
...
float readKv_CP() {
  ...
  float kvcp = (float)Kv_CP_EE / two_to_the(Kv_scale);              // final calculation for Kv_CP_EE. Divide by 2^Kv_scale
#ifdef DEBUG
...
void readTempC() {      // take a temperature reading of all pixels
  ...
  if (subpage == 0) {
    ...
    for (int i = 0; i < NUM_PIXELS; i++) {
      pix_OS_SP0[i] = pix_gain_S0[i] - (float)pix_OS_ref_SP0[i] * (1.0 + Kta[i] * (Ta - 25.0)) * (1.0 + Kv[i] * (Vdd - 3.3));
    }
  ...
  if (subpage == 1) {
    ...
    for (int i = 0; i < NUM_PIXELS; i++) {
      pix_OS_SP1[i] = pix_gain_S1[i] - (float)pix_OS_ref_SP1[i] * (1.0 + Kta[i] * (Ta - 25.0)) * (1.0 + Kv[i] * (Vdd - 3.3));
    }
  ...
  for (int i = 0; i < NUM_PIXELS; i++) {
    ...
    S_x[i] = KsTo3 * fourth_root(powf(alpha_comp[i], 3.0) * V_IR_compensated[i] + powf(alpha_comp[i], 4.0) * Ta_r);     // formula for S_x[i]
    T_o[i] = fourth_root((V_IR_compensated[i] / (alpha_comp[i] * (1.0 - (KsTo3 * 273.15)) + S_x[i])) + Ta_r) - 273.15;  // formula for T_o[i]
...
void setup() {
  ...
  if (!readEEPROMBlock(0x2400, EEPROM_WORDS, eeData)) {
  ...
  for (int i = 0; i < EEPROM_WORDS; i++) {
    if (eeData[i] == 0x0000 || eeData[i] == 0xFFFF) {
      Serial.println("EEPROM value suspicious at address: 0x" + String(0x2400 + i, HEX) + ", value: 0x" + String(eeData[i], HEX));
    }
  }
#endif
  Vdd = readVdd();  // This should be close to 3.3V. Can read once in setup.
  Ta = readTa();    // should happen inside the loop
  Serial.print("Ambient temperature on start: ");
  Serial.println(Ta, 1);  // This should be close to ambient temperature (21°C?)
  readPixelOffset();      // only needs to be read once
  readAlpha();            // read sensitivities (fills alpha_pixel[])
  readKta();              // read Kta coefficients (fills Kta[])
  readKv();               // read Kv coefficients (fills Kv[])
  KsTa = readKsTa();      // read KsTa coefficient
  Serial.println("Finished: read KsTA.");
  readCT();                       // read 8 corner temperatures
  ...
  KTa_CP = readKTa_CP();          // read KTa_CP
  TGC = readTGC();                // read TGC - do this last (leaves setup function for some odd reason)
  ...
  for (int i = 0; i < 192; i++) {
    Serial.print(i);
    Serial.print(", 0x0");
    Serial.print(pix_addr_S0(i), HEX);
    Serial.print(", 0x0");
    Serial.println(pix_addr_S1(i), HEX);
    delay(100);
  }
  #endif*/
...
void loop() {
  ...
  while (millis() - pollStart < 500) {  // 500ms max wait (adjust to > 1000 * 1.2 * (1/refresh_rate in Hz))
    if (isNewDataAvailable()) break;
    delay(10);  // Yield to prevent watchdog/I2C lockup
  }
  if (isNewDataAvailable()) {
    clearNewDataBit();

    readTempC();  // read the temperature
...
bool setRefreshRate(uint8_t rate) {
